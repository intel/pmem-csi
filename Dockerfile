...
# IMAGE_NAME holds the name of the image being built. Possible values are: "driver", "operator"
ARG IMAGE_NAME=driver
...
ARG CACHEBUST
RUN echo "Updating build (${IMAGE_NAME})image from ${CLEAR_LINUX_BASE} to ${SWUPD_UPDATE_ARG:-the latest release}."
RUN swupd update ${SWUPD_UPDATE_ARG} && swupd bundle-add curl c-basic $([ "${IMAGE_NAME}" != "operator" ] && echo ${NDCTL_BUILD_DEPS}) && \
...
WORKDIR /
RUN if [ "${IMAGE_NAME}" != "operator" ]; then \
      curl --fail --location --remote-name https://github.com/pmem/ndctl/archive/v${NDCTL_VERSION}.tar.gz && \
      tar zxvf v${NDCTL_VERSION}.tar.gz && mv ndctl-${NDCTL_VERSION} ndctl; \
    fi
WORKDIR /ndctl
RUN if [ "${IMAGE_NAME}" != "operator" ]; then ./autogen.sh ; fi
...
# also symlink those.
RUN if [ "${IMAGE_NAME}" != "operator" ]; then ./configure --prefix=/usr/local ${NDCTL_CONFIGFLAGS}; fi
RUN if [ "${IMAGE_NAME}" != "operator" ]; then make install && \
      ...
      echo "For source code and licensing of ndctl, see https://github.com/pmem/ndctl/blob/v${NDCTL_VERSION}/COPYING" >/usr/local/lib/NDCTL.COPYING; \
    fi
...
LABEL description="PMEM CSI ${IMAGE_NAME}"
...
RUN swupd update ${SWUPD_UPDATE_ARG} && \
    if [ "${IMAGE_NAME}" != "operator" ]; then \
      swupd bundle-add file xfsprogs storage-utils $([ "$BIN_SUFFIX" = "-test" ] && echo fio) && \
      rm -rf /var/lib/swupd /var/tmp/swupd ;\
    fi
...
# differences.
RUN set -x && \
    # Below emptyfile is used later part of Dockerfile while copying files
    # from 'binaries' -> 'pmem' image to succeed COPY in case of building
    # operator image
    touch /emptyfile && \
    targets=$([ "${IMAGE_NAME}" = "operator" ] && echo "pmem-csi-operator" || echo "pmem-csi-driver pmem-vgm pmem-ns-init") && \
    make VERSION=${VERSION} $(for t in $targets; do echo $t${BIN_SUFFIX}; done) && \
    mkdir -p /usr/local/bin && \
    (for t in $targets; do mv _output/$t${BIN_SUFFIX} /usr/local/bin/$t; done) && \
    (if [ "${IMAGE_NAME}" != "operator" -a "$BIN_SUFFIX" = "-test" ]; then GOOS=linux GO111MODULE=on \
      go build -o /usr/local/bin/pmem-dax-check ./test/cmd/pmem-dax-check; \
    fi) && \
    mkdir -p /usr/local/share/package-licenses && \
    hack/copy-modules-license.sh /usr/local/share/package-licenses $(for t in $targets; do echo ./cmd/$t; done) && \
...
# Incase of building operator image there will b no nodctl
# libraries gneraged, so just to satisify COPY we pass additional
# /emptyfile to COPY so that it will not fail with error
COPY --from=binaries /emptyfile /usr/local/lib/libndctl.so.* /usr/local/lib/libdaxctl.so.* /usr/local/lib/
...
COPY --from=binaries /emptyfile /usr/local/lib/NDCTL.COPYING* /usr/local/share/package-licenses/
# default lvm config uses lvmetad and throwing below warning for all lvm tools
# WARNING: Failed to connect to lvmetad. Falling back to device scanning.
# So, ask lvm not to use lvmetad
RUN if [ ${IMAGE_NAME} != "operator" ]; then \
      mkdir -p /etc/lvm && \
      echo "global { use_lvmetad = 0 }" >> /etc/lvm/lvm.conf && \
