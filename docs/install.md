...
If your cluster supports managing the operators using [Operator Life Manager](https://olm.operatorframework.io/),
would recommend to install the PMEM-CSI operator from the [OperatorHub](https://operatorhub.io/operator/pmem-csi-operator).
...
**NOTE:** Refer to [PMEM-CSI operator API conversion webhook](#pmem-csi-operator-api-conversion-webhook) section
...
The operator release v0.9.0 deprecates the `v1alpha1` [deployment API](#pmem-csi-deployment-crd)
in favor of `v1beta1`. To keep supporting a custom resource that uses the
`v1alpha1` version must be converted to the latest supported `v1beta1` CRD
version. This conversion is done by the Kubernetes API server by using the
conversion strategy chosen in the [deployment CRD specification](#pmem-csi-deployment-crd).
If no conversion strategy is chosen by the CRD, by default the API server
converts the custom resource `apiVersion` to the latest supported CRD version.

This default conversion works as long as the deployment custom resource has no
fields that were deprecated in the latest version. Such as `v1beta1` deprecates
`nodeResources` and `controllerResources` in favor of per-container resource
requests. These fields are silently ignored.
...
deployment.pmem-csi.intel.com/pmem-csi.intel.com created
...
To support auto-migrating of deprecated(v1alpha1) API versions, The operator
could be deployed to run a built-in CRD [conversion webhook](https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definition-versioning/)
server. By default, it starts an HTTP server at port number 9443 and serving
traffic on path `/convert`. The conversion service is exposed via a Kubernetes
service named `pmem-cis-operator-service` in the operator's namespace.
...
object named `pmem-csi-operator-service-cert` in the operator's namespace. And
make sure that the `spec.conversion.webhook.caBundle` field of the
`deployments.pmem-csi.intel.com` CRD is provided with the CA bundle(base64 encoded)
used for signing the certificate.

The provided [operator deployment yaml](/deploy/operator/pmem-csi-operator-webhook.yaml)
uses `cert-manager` for providing the certificates and injecting the CA bundle into the CRD.
...
With the above setup, the deprecated deployment API version will automatically
gets converted to an appropriate new version. The below example shows that the
`nodeResources` and `controllerResources` of `v1alpha1` API are converted to
equivalent `v1beta1` API fields, i.e.,`nodeDriverResources` and
`controllerDriverResources` respectively.
...
**NOTE**: Conversion webhook is not yet supported by the operator installed using the
[Operator Lifecycle Manager](https://operatorhub.io/operator/pmem-csi-operator).
If the operator is deployed with no conversion webhook then it will not
support the conversion of deprecated API fields as described above.
In that case, the user has to manually migrate their existing driver deployments
to use the latest operator API.
