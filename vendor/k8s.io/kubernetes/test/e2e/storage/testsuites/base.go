...
func CreateVolumeResource(driver TestDriver, config *PerTestConfig, pattern testpatterns.TestPattern) *VolumeResource {
	r := VolumeResource{
		Config:  config,
		Pattern: pattern,
	}
	...
	case testpatterns.CSIInlineVolume:
		e2elog.Logf("Creating resource for CSI ephemeral inline volume")
	...
	default:
		e2elog.Failf("VolumeResource doesn't support: %s", pattern.VolType)
	}
...
func (r *VolumeResource) CleanupResource() {
