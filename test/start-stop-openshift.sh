...
#!/bin/sh
sudo podman run -u 0:0 -i --privileged --rm docker.io/intel/pmem-csi-driver:v1.0.2 ndctl "\$@"
EOF
...
init_pmem_regions() {
    for domain in $($VIRSH list --all --name | grep ^$TEST_VIRT_CLUSTER_NAME-.*worker); do
        run ssh $SSH_ARGS core@$domain.$TEST_VIRT_CLUSTER_NAME.$TEST_VIRT_CLUSTER_DOMAIN \
            sudo podman run -u 0:0 -i --privileged --rm docker.io/intel/pmem-csi-driver:v1.0.2 /bin/sh <<EOF
set -xe
